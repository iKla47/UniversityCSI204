import{r as e,t}from"./react-C1VktWof.js";import{t as n}from"./index-Ck9etSGg.js";import{r}from"./styled-components.browser.esm-BhrFqjok.js";import{t as i}from"./circle-user-bdA1tqbP.js";import{g as a,r as o}from"./customer-Dw5Nd6pM.js";import{_ as s,g as c,i as l,y as u}from"./common.ui-xMs2_JMV.js";var d=`data:image/x-icon;base64,`,f=e(t(),1),p=n(),m=function(e){let t=()=>{a({width:n()})},n=()=>r.current instanceof HTMLDivElement&&r.current!=HTMLDivElement.prototype?r.current.clientWidth:0,r=f.useRef(HTMLDivElement.prototype),[i,a]=f.useState({width:n()});return f.useLayoutEffect(()=>{t()},[]),f.useEffect(()=>(window.addEventListener(`resize`,t),()=>{window.removeEventListener(`resize`,t)}),[]),(0,p.jsx)(h,{ref:r,children:(0,p.jsx)(l,{value:i,children:e.children})})};m.Branding=function(e){let t=c().width>=1268;return(0,p.jsxs)(g,{onClick:t=>{t.preventDefault(),t.stopPropagation(),e.onClick&&e.onClick()},children:[(0,p.jsx)(_,{src:e.icon}),(0,p.jsx)(v,{$show:t,children:e.text})]})},m.Search=function(e){let t=(0,f.useRef)(null);return(0,p.jsx)(y,{ref:t,placeholder:e.placeholder,onClick:n=>{n.preventDefault(),n.stopPropagation(),e.onClick&&t.current&&e.onClick(t.current.value)},onChange:t=>{e.onChange&&e.onChange(t.target.value)}})},m.Profile=function(e){let{data:t}=o();return(0,p.jsxs)(b,{onClick:t=>{t.preventDefault(),t.stopPropagation(),e.onClick&&e.onClick()},children:[!e.icon&&!t?(0,p.jsx)(i,{}):(0,p.jsx)(p.Fragment,{}),!e.icon&&t?t.icon.length>0?(0,p.jsx)(`img`,{src:a.getUrlStream(t.icon)}):(0,p.jsx)(i,{}):(0,p.jsx)(p.Fragment,{}),e.icon?(0,p.jsx)(`img`,{src:e.icon}):(0,p.jsx)(p.Fragment,{})]})},m.SignIn=function(e){return(0,p.jsx)(x,{onClick:t=>{t.preventDefault(),t.stopPropagation(),e.onClick&&e.onClick()},children:`ลงชื่อเข้าใช้`})},m.Menu=function(e){let t=c(),n=e.hideOnWidth?e.hideOnWidth<=t.width:!0;return(0,p.jsx)(S,{$visible:n,children:e.children})},m.MenuItem=function(e){let t=c(),n=e.hideOnWidth?e.hideOnWidth<=t.width:!0,r=e.icon;return(0,p.jsxs)(C,{$visible:n,onClick:t=>{t.preventDefault(),t.stopPropagation(),e.onClick&&e.onClick()},children:[(0,p.jsx)(()=>typeof r==`string`?(0,p.jsx)(`img`,{src:r,alt:``}):f.isValidElement(r)?r:typeof r==`function`||typeof r==`object`?(0,p.jsx)(r,{}):null,{}),t.width>=768?e.text:``]})},m.Spacing=function(e){return(0,p.jsx)(w,{$level:e.level??1})};var h=r.div`
  position: fixed;
  inset: 0px 0px auto 0px;
  height: 48px;

  display: flex;
  flex-direction: row;
  flex-wrap: nowrap;
  gap: 8px;

  align-items: center;

  background-color: var(--bg-primary);
  border-color: var(--bg-primary-border);
  border-width: 0px 0px 2px 0px;
  border-style: solid;

  & > *:first-child { margin: 0px 0px 0px 32px; }
  & > *:last-child { margin: 0px 32px 0px 0px; }

  @media (max-width: 768px)
  {
    & > *:first-child { margin: 0px 0px 0px 16px; }
    & > *:last-child { margin: 0px 16px 0px 0px;; }
  }
`,g=r.button`
  min-width: 32px;
  max-height: 32px;
  padding: 0px 8px;
  background-color: transparent;
  border: transparent;

  display: flex;
  flex-direction: row;
  flex-wrap: nowrap;
  align-items: center;
  margin: 0px;
  padding: 0px;
`,_=r.img`
  width: 32px;
  height: 32px;
  vertical-align: middle;
  display: inline-block;
`,v=r.label`
  width: auto;
  font-size: 1.25rem;
  font-weight: normal;
  padding: 0px 16px;
  display: ${e=>e.$show?`block`:`none`};
  visibility: ${e=>e.$show?`visible`:`hidden`};
  pointer-events: ${e=>e.$show?`all`:`none`};
`,y=r.input`
  display: block;
  max-width: 256px;
  min-height: 32px;

  font-size: 1rem;
  background-color: #418C94;
  border-radius: 16px;
  border: none;
  outline: none;
  color: white;
  padding: 0px 16px;

  &::placeholder
  {
    color: #9ad3cb; 
  }
`,b=r.button`
  display: block;
  margin: 0px;
  padding: 0px;
  width: 32px;
  min-height: 32px;

  background-color: transparent;
  color: var(--text-primary);

  &:hover, &:focus
  {
    background-color: transparent;
    color: var(--text-primary);
  }
  &:active
  {
    background-color: transparent;
    color: var(--text-primary);
  }

  & > img,
  & > svg
  {
    display: block;
    width: 32px;
    height: 32px;
    color: var(--text-primary);
    background-color: var(--bg-secondary);
    border: 0px solid var(--bg-primary-border);
    border-radius: 100%;
    transition: border-width 66ms cubic-bezier(0.22, 1, 0.36, 1);
  }
  & > img:hover,
  & > svg:hover
  {
    border-width: 2px;
  }
`,x=r.button`
  min-height: 32px;
  max-height: 32px;
  font-size: 1rem;
  padding: 0px 16px;
  background-color: var(--btn-primary);
  border-radius: 4px;
  border: none;
  outline: none;
  color: var(--btn-primary-text);

  &:hover, &:focus {
    background-color: var(--btn-primary-hover);
    color: var(--btn-primary-hover-text);
  }
  &:active {
    background-color: var(--btn-primary-active);
    color: var(--btn-primary-active-text);
  }
  @media (max-width: 1024px)
  {
    min-width: 128px;
  }
`,S=r.div`
  margin: 0px 8px;
  max-height: 32px;
  display: ${e=>e.$visible?` inline-flex`:`none`};
`,C=r.button`
  min-width: 32px;
  min-height: 32px;
  max-height: 32px;
  background-color: transparent;
  border: transparent;
  display: ${e=>e.$visible?`flex`:`none`};
  flex-direction: row;
  flex-wrap: nowrap;
  gap: 16px;
  align-items: center;
  justify-content: center;

  &:hover, &:focus
  {
    background-color: transparent;
    color: var(--text-primary-hover);
  }
  &:active
  {
    background-color: transparent;
    color: var(--text-primary-active);
  }

  & > img,
  & > svg
  {
    min-width: 24px;
    min-height: 24px;
    max-width: 24px;
    max-height: 24px;
  }
  color: var(--text-primary);
  font-size: 1rem;
`,w=r.div`
  min-height: 32px;
  flex-grow: ${e=>e.$level}
`;Object.freeze(m);var T=`data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAACXBIWXMAAC4jAAAuIwF4pT92AAAAEklEQVQ4y2NgGAWjYBSMAggAAAQQAAGFP6pyAAAAAElFTkSuQmCC`,E=function(e){return(0,p.jsx)(E.Root,{visible:e.visible,inset:e.inset,width:e.width,widthMax:e.widthMax,height:e.height,heightMax:e.heightMax,onCancel:e.onCancel,children:e.children})};E.Root=function(e){let t=f.useRef([0,0]),n=`${String(t.current[0])}px`,r=`${String(t.current[1])}px`,i=e.visible??!1,a=e.inset??`${r} auto auto ${n}`,o=e.width??`256px`,s=e.widthMax??`256px`,c=e.height??`auto`,l=e.heightMax??`384px`,u=t=>{t.preventDefault(),t.stopPropagation(),e.onCancel&&e.onCancel()},d=e=>{e.preventDefault(),e.stopPropagation()},m=e=>{t.current=[e.clientX,e.clientY]};return f.useEffect(()=>(window.addEventListener(`mousemove`,m),()=>{window.removeEventListener(`mousemove`,m)}),[]),(0,p.jsx)(f.Activity,{mode:i?`visible`:`hidden`,children:(0,p.jsx)(D,{children:(0,p.jsx)(O,{onClick:u,children:(0,p.jsx)(k,{$inset:a,$width:o,$widthMax:s,$height:c,$heightMax:l,children:(0,p.jsx)(A,{onClick:d,$inset:a,$width:o,$height:c,children:e.children})})})})})},E.Item=function(e){let t=e.icon;return(0,p.jsxs)(j,{onClick:t=>{t.preventDefault(),t.stopPropagation(),e.onClick&&e.onClick()},children:[(0,p.jsx)(()=>typeof t==`string`?(0,p.jsx)(`img`,{src:t,alt:``}):f.isValidElement(t)?t:typeof t==`function`||typeof t==`object`?(0,p.jsx)(t,{}):(0,p.jsx)(`img`,{src:T,alt:``}),{}),e.text]})},E.Provider=function(){let e=s(),[t,n]=f.useState(!1),[r,i]=f.useState((0,p.jsx)(p.Fragment,{})),[a,o]=f.useState(void 0),c=f.useRef(void 0);return f.useEffect(()=>(e.setVisible=e=>{n(e)},e.setChildren=e=>{i(e)},e.setInset=e=>{o(e)},e.setCancel=e=>{c.current=e},()=>{e.setVisible=()=>{},e.setChildren=()=>{},e.setInset=()=>{}}),[]),(0,p.jsx)(E.Root,{visible:t,inset:a,onCancel:c.current,children:r})};var D=r.div`
  overflow: hidden;
  overscroll-behavior: none;
  pointer-events: all;
  position: fixed;
  inset: 0px;
`,O=r.div`
  position: relative;
  width: 100%;
  height: 100%;
`,k=r.div`
  position: absolute;
  inset: ${e=>e.$inset};
  width: ${e=>e.$width};
  height: ${e=>e.$height};
  max-width: ${e=>e.$widthMax};
  max-height: ${e=>e.$heightMax};
`,A=r.div`
  inset: ${e=>e.$inset};
  width: ${e=>e.$width};
  height: ${e=>e.$height};
  max-width: 100%;
  max-height: 100%;

  background-color: var(--bg-primary);
  border: 2px solid var(--bg-primary-border);
  border-radius: 4px;
`,j=r.button`
  background-color: transparent;
  color: var(--text-primary);
  display: block;
  width: 100%;
  text-align: start;

  & > img,
  & > svg
  {
    display: inline-block;
    vertical-align: middle;
    margin-right: 16px;
    width: 24px;
    height: 24px;
  }
`,M=function(){return M.Root};M.Root=function(e){return(0,p.jsx)(N,{children:(0,p.jsx)(P,{children:(0,p.jsx)(F,{$visible:e.visible??!1,children:(0,p.jsx)(`p`,{children:e.text})})})})},M.Provider=function(){let[e,t]=(0,f.useState)(!1),[n,r]=(0,f.useState)(``),[i,a]=(0,f.useState)(``),o=(0,f.useRef)(0),s=(0,f.useRef)(0),c=u(),l=()=>{clearTimeout(s.current),s.current=setTimeout(()=>{t(!1)},o.current)};return(0,f.useEffect)(()=>(c.setIcon=e=>{r(e)},c.setText=e=>{a(e)},c.setDuration=e=>{o.current=e},c.setVisible=e=>{t(e),l()},()=>{c.setIcon=()=>{},c.setText=()=>{},c.setDuration=()=>{},c.setVisible=()=>{}}),[c]),(0,f.useEffect)(()=>()=>{clearTimeout(s.current)},[]),(0,p.jsx)(M.Root,{visible:e,icon:n,text:i})};var N=r.div`
  display: block;
  pointer-events: none;
  position: fixed;
  inset: 0px;
  z-index: 1000;
`,P=r.div`
  position: relative;
  width: 100%;
  height: 100%;
`,F=r.div`
  position: absolute;
  inset: auto auto 16px 16px;
  width: auto;
  height: 40px;
  background-color: var(--bg-primary);
  border-radius: 4px;
  display: flex;
  flex-direction: row;
  flex-wrap: nowrap;
  gap: 16px;
  align-items: center;
  padding: 0px 16px;

  transform: translateY(${e=>e.$visible?`0px`:`64px`});
  transition: transform 250ms linear;
`;export{d as i,E as n,m as r,M as t};